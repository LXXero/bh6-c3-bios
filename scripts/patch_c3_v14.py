...
assert all(x==0xFF for x in b[CAVE:CAVE+0x240])
def put(off,data):
...
# (5) Clock-gen apply gate 0xC658: `mov al,0xB9; call rd; test al,1; jnz skip` -> never skip on the nag bit
assert bytes(b[0xC658:0xC661]) == bytes.fromhex('b0b9e8f0c1a8017528'), "apply gate moved"
b[0xC65F:0xC661] = b'\x90\x90'
...
# (7) TRACE: snapshot CMOS C1 (FSB byte) entering phase 0x1A11 -> CMOS 0x33, and
#     entering clock-gen apply phase 0x1C11 -> CMOS 0x34, by repointing the phase-table
#     words (0x8E64 -> 0x1A11 stub, 0x8E6C -> 0x1C11 stub) to cave trampolines.
TA = CAVE + 0x1C0   # trace before phase 0x1A11 -> reg 0x33
TB = CAVE + 0x1E0   # trace before phase 0x1C11 -> reg 0x34
assert int.from_bytes(b[0x8E65:0x8E67],'little')==0x8E95, "phase tbl word0 moved"
assert int.from_bytes(b[0x8E6D:0x8E6F],'little')==0x8EA5, "phase tbl word4 moved"
assert all(x==0xFF for x in b[TA:TA+0x40]), "cave collision TA/TB"
def tramp(addr, wreg, phase):
    f=bytearray()
    f+=bytes([0xB0,0xC1]); f+=bytes([0xE8])+struct.pack('<H',rel16(addr+len(f)+3,0x884D))  # al=CMOS C1
    f+=bytes([0x8A,0xE0])                                                                   # mov ah,al
    f+=bytes([0xB0,wreg]); f+=bytes([0xE8])+struct.pack('<H',rel16(addr+len(f)+3,0x885F))   # CMOS[reg]=ah
    f+=bytes([0xE8])+struct.pack('<H',rel16(addr+len(f)+3,phase))                           # call real phase
    f+=bytes([0xC3])                                                                        # ret (CF preserved)
    return bytes(f)
put(TA, tramp(TA,0xB3,0x1A11))
put(TB, tramp(TB,0xB4,0x1C11))
struct.pack_into("<H", b, 0x8E65, TA)
struct.pack_into("<H", b, 0x8E6D, TB)
