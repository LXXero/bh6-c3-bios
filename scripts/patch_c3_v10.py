...
assert all(x==0xFF for x in b[CAVE:CAVE+0x1A0])
def put(off,data):
...
# (4) Kill the sticky "CPU is unworkable or has been changed" nag/reset:
#     validator entry 0x44: `test byte [bp+0x39],1 ; jnz reset` -> `and byte [bp+0x39],0xFE ; nop ; nop`
#     ([bp+0x39] = CMOS 0x39 shadow; the end-of-POST writeback persisted the bit forever.)
assert bytes(b[0x44:0x4A]) == bytes.fromhex('f64639017524'), "nag check moved"
b[0x44:0x4A] = bytes.fromhex('806639fe9090')
