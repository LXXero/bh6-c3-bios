#!/usr/bin/env python3
"""BH6 SP -> VIA C3: (1) vendor detect + POST name, (2) VIA EBLCR multiplier decode.
...
# (4) Kill the sticky "CPU is unworkable or has been changed" nag/reset, in RAM *and* CMOS:
#     validator entry 0x44: `test byte [bp+0x39],1 ; jnz reset` (6 bytes) -> `call NAGCLR ; nop x3`
#     NAGCLR (cave): and byte [bp+0x39],0xFE ; mov ah,[bp+0x39] ; mov al,0xB9 ; call 0x885F (BIOS CMOS write) ; ret
NAGCLR = CAVE + 0x1A0
assert bytes(b[0x44:0x4A]) == bytes.fromhex('f64639017524'), "nag check moved"
assert bytes(b[0x2AC:0x2B1]) == bytes.fromhex('b0b9e8ae85'), "0x885F CMOS-write helper usage changed"
put(NAGCLR, bytes.fromhex('806639fe') + bytes.fromhex('8a6639') + bytes.fromhex('b0b9')
            + bytes([0xE8]) + struct.pack('<H', rel16(NAGCLR+9+3, 0x885F)) + bytes([0xC3]))
b[0x44:0x4A] = bytes([0xE8]) + struct.pack('<H', rel16(0x47, NAGCLR)) + b'\x90\x90\x90'

# (5) Clock-gen apply gate 0xC658 (stock: mov al,0xB9; call rd; test al,1; jnz skip).
#     v15: repurpose this dead-code slot to `call FSBFIX` so that, right before the apply
#     reads CMOS 0x41 to build the ICS9148 FSB mode byte (0xC672), a C3 gets CMOS 0x41
#     forced to External-Clock index 22 = "100MHz(1/3)S" (mode byte 0x72: 100MHz host,
#     SEL100/66# asserted -> 443BX straps AGP 2/3 = 66MHz, PCI 1/3 = 33MHz), and CMOS 0x3A
#     bit6 (AGPCLK/CPUCLK = 2/3) set. This is downstream of every SoftMenu cold-boot reset of
#     0x41, so the synth latches a true 100MHz regardless of what the preset engine recomputed.
assert bytes(b[0xC658:0xC661]) == bytes.fromhex('b0b9e8f0c1a8017528'), "apply gate moved"
FSBFIX = CAVE + 0x1C0                     # 0xD7C0, verified FF-free in the v12 layout
def rel8(end,to):
    d=(to-end)&0xFF
    assert -128 <= ((d^0x80)-0x80) <= 127
    return d
f=bytearray(); A=lambda: FSBFIX+len(f)
f+=bytes([0xE8])+struct.pack('<H',rel16(A()+3,CENTCHK))   # call CENTCHK (ZF=1 if CentaurHauls)
jnz_at=len(f); f+=bytes([0x75,0x00])                      # jnz done  (patch target below)
f+=bytes([0xB0,0xC1, 0xB4,0xB7])                          # mov al,0x41 ; mov ah,0xB7 (idx22|SEL)
f+=bytes([0xE8])+struct.pack('<H',rel16(A()+3,0x885F))    # call CMOS-write
f+=bytes([0xB0,0xBA])                                     # mov al,0x3A
f+=bytes([0xE8])+struct.pack('<H',rel16(A()+3,0x884D))    # call CMOS-read -> al
f+=bytes([0x0C,0x40])                                     # or al,0x40 (AGPCLK 2/3)
f+=bytes([0x8A,0xE0, 0xB0,0xBA])                          # mov ah,al ; mov al,0x3A
f+=bytes([0xE8])+struct.pack('<H',rel16(A()+3,0x885F))    # call CMOS-write
done=len(f); f+=bytes([0xC3])                             # done: ret
f[jnz_at+1]=rel8(FSBFIX+jnz_at+2, FSBFIX+done)
put(FSBFIX, bytes(f))
# hook: overwrite the 9 dead bytes at 0xC658 with `call FSBFIX` + NOP padding
b[0xC658:0xC661] = bytes([0xE8])+struct.pack('<H',rel16(0xC65B,FSBFIX))+b'\x90'*6

# (6) Generic CMOS writeback (E000:9EB3) first range 0x10..0x3F -> 0x10..0x41, so the ABIT
#     FSB/SEL100/66 byte (CMOS 0x41) is carried from the shadow into CMOS on save & at boot.
#     Checksum ranges (table F000:0B90) are separate and unchanged.
assert bytes(b[0x9F20:0x9F27]) == bytes.fromhex('bf900bb510b13f'), "writeback range code moved"
b[0x9F26] = 0x41
